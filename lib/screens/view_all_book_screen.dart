...
class _ViewAllBooksState extends State<ViewAllBooks>  with AfterLayoutMixin<ViewAllBooks>{
  ...
  Future<List<BookDetail>> fetchBookList(page) async {
    ...
    isNetworkAvailable().then((bool) {
      if (bool) {
        getViewAllBookNextPage(widget.type,page,categoryId: widget.categoryId).then((result) {
    ...
    final appBar = AppBar(
      ...
      title: Column(
        ...
        children: <Widget>[
          Text(
            widget.title,
          ).withStyle(fontSize: ts_extra_normal, color: Theme.of(context).textTheme.title.color, fontFamily: font_bold).paddingTop(spacing_standard_new),
          Text(totalBooks.toString()+' '+keyString(context,"lbl_books")).withStyle(fontSize: ts_medium, fontFamily: font_regular, color: Theme.of(context).textTheme.subtitle.color).paddingTop(spacing_control_half)
        ],
      ),
    );
        ...
        Stack(
          ...
          children: <Widget>[
            SingleChildScrollView(
              ...
              child: list.isNotEmpty?isLoadingMoreData?Column(
                children: <Widget>[
                  books.paddingTop(spacing_standard_new),
                  loadingWidgetMaker()
                ],
              ):books.paddingTop(spacing_standard_new):Center(child: loadingWidgetMaker().paddingTop(spacing_large).visible(isLoadingMoreData))),
            Center(
              child: text(context,keyString(context,"error_no_result"),fontSize: ts_extra_normal,textColor: Theme.of(context).textTheme.title.color),
